...
#' and "EXP3-Bayes-Poisson-TS"; \code{iter.BP} specifies the number of iterations
...
#' of the number of arms. Available distribution includes "Poisson" and "Binomial".
...
#' \code{shape2} are two shape parameters when \eqn{reward.mean.family = "Beta"};
...
#'  when \eqn{npulls.family = "Log-Normal"}; \code{lambda} is a positive parameter
#' specifying the mean when \eqn{npulls.family = "Poisson"}.
#'
#'
#' \code{nonstationary.family}: A list of parameters of \code{nonstationary.type}:
...
#' set.seed(100)
#' res <- SimulateMultipleMethods(method = c("Epsilon-Greedy", "Thompson-Sampling"),
...
#' set.seed(100)
#' res <- SimulateMultipleMethods(method = c("Epsilon-Greedy", "Thompson-Sampling", "EXP3"),
...
SimulateMultipleMethods <- function(method = "Thompson-Sampling",
  ...
  for (i in 1:iter){
    ...
    if (npulls.family == "Log-Normal"){
      npulls.per.period <- ceiling(rlnorm(nburnin + nperiod,
